...
export default {
  ...
  methods: {
    ...
    transitionTo (index, duration = 300) {
      this.clearTimer()
    ...
    slid (index) {
      let { childrenLength: len, computedLoop, height, transitionDuration, direction, $slots } = this
      if (len === 0)  return
      if (!computedLoop) {
        index = (index + len) % len
      }
      let translateY = -height * (index + (computedLoop ? 1 : 0))
      this.trackStyle = {
  ...
  render (h) {
    ...
    let itemStyle = {
      height: this.height + (typeof this.height === 'number' ? 'px' : '')
    }
    return (
      <div class="container">
